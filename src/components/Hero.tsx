...
export const Hero: React.FC<HeroProps> = ({ isLoaded, onSetCursor, onOpenTerminal }) => {
  ...
  return (
    <section id="hero" className="relative min-h-screen pt-28 pb-16 px-4 sm:px-8 flex flex-col justify-between overflow-hidden bg-[#0a0a0a]">
      {/* Background brutalist grid lines */}
      <div className="absolute inset-0 border-x border-white/5 max-w-7xl mx-auto pointer-events-none flex justify-between">
      ...
      <div className="max-w-7xl mx-auto w-full flex flex-wrap items-center justify-between gap-4 py-4 border-b border-white/10 font-mono text-xs z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-neutral-400">
            FRAMEWORK: <strong className="text-white">REACT 19 + TAILWIND V4</strong>
          </span>
        </div>

        <div className="flex items-center gap-4 text-neutral-400">
          <span className="hidden md:inline">LATENCY: <strong className="text-green-400">12MS</strong></span>
          <button
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-x-1.5 font-mono text-xl sm:text-2xl md:text-3xl font-extrabold text-neutral-300">
              ...
              {roleLetters.map((char, i) => (
                <span
                  key={i}
                  className={char === " " ? "w-3" : "hover:text-[#70020F] transition-colors cursor-pointer"}
                >
                  {char}
                </span>
              ))}
